...
export const SPLINE_CURVE_TYPES = ['linear', 'centripetal', 'chordal', 'catmullrom'] as const
export type CurveType = (typeof SPLINE_CURVE_TYPES)[number]

export type SplineElementProps = {
...
export const SplineElement = forwardRef<THREE.Group, SplineElementProps>(function SplineElement({
  ...
  const curvePoints = useMemo(() => {
    ...
    if (curveType === 'linear') {
      // For linear, subdivide each segment evenly
      const result: THREE.Vector3[] = []
      for (let i = 0; i < vectors.length - 1; i++) {
        const a = vectors[i]
        const b = vectors[i + 1]
        const segsPerEdge = Math.max(1, Math.round(segments / Math.max(1, vectors.length - 1)))
